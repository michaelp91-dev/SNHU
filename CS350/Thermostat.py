...
##
ser = serial.Serial(
        port='/dev/serial0', # This would be /dev/ttyAM0 prior to Raspberry Pi 3
...
class TemperatureMachine(StateMachine):
    ...
    def manageMyDisplay(self):
        ...
        while not self.endDisplay:
            ...
            state_str = self.current_state.id.upper()[:4].ljust(4)
            lcd_line_1 = current_time.strftime("%m/%d %H:%M ") + state_str + "\n"
    
            ## Setup Display Line 2
            temp = floor(self.getFahrenheit())
            active = "ON" if (
                (self.current_state == self.heat and temp < self.setPoint) or
                (self.current_state == self.cool and temp > self.setPoint)
            ) else "OFF"
            lcd_line_2 = f"TEMP{temp} SP{self.setPoint} {active}\n"
